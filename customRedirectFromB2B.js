(function () {
  ...
  var formCallbackList = {
    ...
    'form594612135': callLessonRegListeners,
    'form594917042': callQuizRegListeners
  }
  ...
  async function formSuccessHandler(form) {
    ...
    async function callLessonRegListeners(form) {
        localLessonRegCallback(form)
        submitLessonReg(form)
    }
    
    async function callQuizRegListeners(form) {
        await localQuizRegCallback(form)
        submitQuizReg()
    }
  ...
  function submitLessonReg(form) {
      email = $(`#${form.id} input[name="email"]`).val();       
      name = $(`#${form.id} input[name="name"]`).val();
      phone = $(`#${form.id} input[name="phone"]`).val().replace(/[^\+/s0-9]/g, '');
      email = $(`#${form.id} input[name="email"]`).val();
    ...
    function localQuizRegCallback(form) {
    var xhr = new XMLHttpRequest()
